...
/// Scalar forward NTT implementation.
fn nttScalar(a: *[N]i32) void {
    var k: usize = 0;
    var len: usize = 128;
    ...
    while (len >= 1) : (len >>= 1) {
        ...
        while (start < N) {
            ...
            while (j < start + len) : (j += 1) {
                const t = montgomeryReduce(@as(i64, zeta) * @as(i64, a[j + len]));
                a[j + len] = a[j] - t;
                a[j] = a[j] + t;
            }
            start = j + len;
...
/// Scalar inverse NTT implementation.
fn invnttTomontScalar(a: *[N]i32) void {
    var k: usize = 256;
    var len: usize = 1;
    ...
    while (len < N) : (len <<= 1) {
        ...
        while (start < N) {
            ...
            while (j < start + len) : (j += 1) {
                ...
                a[j] = t + a[j + len];
                a[j + len] = t - a[j + len];
                a[j + len] = montgomeryReduce(@as(i64, zeta) * @as(i64, a[j + len]));
            }
            start = j + len;
        }
    }
    ...
    for (a) |*coeff| {
        coeff.* = montgomeryReduce(@as(i64, f) * @as(i64, coeff.*));
    }
}
...
test "ntt roundtrip" {
    ...
    // Simple test: NTT followed by inverse NTT should recover original (in Montgomery form)
    ...
    // After NTT -> INTT, we get original * 2^32 mod Q (Montgomery form)
    // For a basic sanity check, just verify we get valid field elements
    for (poly) |c| {
